...
/*
    ...
    * 动态分配：
    int pthread_mutex_init(pthread_mutex_t * restrict mutex,const pthreead_mutexattr_t* restrict attr)
...
/*
    ...
    * 不使用本地数据，或者通过制作全局数据的本地拷贝来保护全局数据 
     ...
     # 常见不可重入的情况
     * 使用malloc或free  因为malloc是用全局链来管理堆的
     * 调用表尊IO，
     * 可重入函数中使用静态的数据结构
...
/*
    ...
    * 请求和保持条件：一个执行流因申请资源而阻塞时，对以获得的资源保持不放
